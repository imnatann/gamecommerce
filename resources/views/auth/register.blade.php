...
@section('content')
<div x-data="{ name: '', email: '', phone: '', password: '', passwordConfirmation: '', showPassword: false, loading: false }">
    <h2 class="font-[var(--font-family-display)] text-2xl font-bold text-[var(--color-gc-text)] text-center mb-2">Buat Akun Baru</h2>
    <p class="text-sm text-[var(--color-gc-text-secondary)] text-center mb-6">Bergabung bersama GameCommerce sekarang!</p>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-4">
        ...
        <div>
            <label for="name" class="block text-sm font-medium text-[var(--color-gc-text-secondary)] mb-1">Nama Lengkap</label>
            <input id="name" type="text" name="name" x-model="name" class="gc-input w-full" placeholder="Nama lengkap" required autofocus autocomplete="name">
            @error('name')
        ...
        <div>
            ...
            <input id="email" type="email" name="email" x-model="email" class="gc-input w-full" placeholder="nama@email.com" required autocomplete="email">
            @error('email')
                <p class="text-xs text-[var(--color-gc-error)] mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            <label for="phone" class="block text-sm font-medium text-[var(--color-gc-text-secondary)] mb-1">Nomor Telepon</label>
            <input id="phone" type="tel" name="phone" x-model="phone" class="gc-input w-full" placeholder="08xxxxxxxxxx" autocomplete="tel">
            @error('phone')
                <p class="text-xs text-[var(--color-gc-error)] mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <div class="relative">
                <input id="password" :type="showPassword ? 'text' : 'password'" name="password" x-model="password" class="gc-input w-full pr-10" placeholder="Minimal 8 karakter" required autocomplete="new-password">
        ...
        <div>
            <label for="password_confirmation" class="block text-sm font-medium text-[var(--color-gc-text-secondary)] mb-1">Konfirmasi Password</label>
            <input id="password_confirmation" type="password" name="password_confirmation" x-model="passwordConfirmation" class="gc-input w-full" placeholder="Ulangi password" required autocomplete="new-password">
        ...
        <div class="flex items-start gap-2">
            <input id="terms" type="checkbox" name="terms" class="gc-checkbox mt-0.5" required>
            <label for="terms" class="text-xs text-[var(--color-gc-text-secondary)]">
                Saya setuju dengan <a href="#" class="text-[var(--color-gc-primary)] hover:underline">Syarat & Ketentuan</a> dan <a href="#" class="text-[var(--color-gc-primary)] hover:underline">Kebijakan Privasi</a>
            </label>
        </div>
